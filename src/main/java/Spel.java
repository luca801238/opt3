...
public class Spel {
    ...
    public void verwerkCommando(String input) {
        if (input.startsWith("ga naar kamer")) {
            ...
            if (huidigeKamerIndex + 1 >= kamers.size()) {
                System.out.println("🎉 Je hebt alle kamers doorlopen en het spel uitgespeeld! Goed gedaan!");
                System.exit(0);
            }

            if (!speler.heeftMonster() && gevraagdNummer == huidigeKamerIndex + 2 && gevraagdNummer <= kamers.size()) {
                huidigeKamerIndex++;
        ...
        if (!opdrachtGestart) {
            ...
            if (speler.getHuidigeKamer().checkAntwoord(input)) {
                ...
                // Check of dit de laatste kamer was
                if (huidigeKamerIndex + 1 == kamers.size()) {
                    System.out.println("🎉 Je hebt alle kamers doorlopen en het spel uitgespeeld! Goed gedaan!");
                    System.exit(0);
                }
